...
DESCRIPTION
    ...
    This is the offspring generation variant algorithm, which is a variant
    for how new populations are created.

    Solution Representation:
        - Initially 10 three vertex polygons in an individual
    Mutation:
        - Add polygon to individual
        - Add new vertex to a polygon in individual
...
def select(population):
    ...
    Uses roulette wheel selection where each individual weighting is
    based on the fitness of that individual. Those with a better fitness
    are more likely to be selected for reproduction and vice versa.

    :param population: List of individuals representing the population
    :return: A list containing two individuals
    """

    # Sum value of the populations fitness
    fitness_sum = sum(population[x].fitness for x in range(len(population)))
    # Calculating the probability of an individual based on its fitness
    selection_probs = [population[y].fitness / fitness_sum for y in range(len(population))]

    return [population[npr.choice(len(population), p=selection_probs)],
            population[npr.choice(len(population), p=selection_probs)]]
...
def mutate(individual, rate):
    ...
    if roulette < 0.2:
        # Add polygon to individuals chromosome
        if len(individual) < MAX_POLYGON_COUNT:
            individual.append(initialize_polygon())
    elif 0.2 < roulette < 0.4:
        # Add point to polygon
        individual[polygon_index].append((random.randint(0, 200),  # x3
                                          random.randint(0, 200)))

        coords = individual[polygon_index][1:]
        centre_points = centroid(coords)
        coord_atan = []

        # Calculate atan of the coords in relation to centrepoint
        for coord in coords:
            coord_atan.append(math.atan2(coord[1]-centre_points[1], coord[0]-centre_points[0]))

        # Sort coords based on atan
        for i in range(len(coord_atan)):
            min_idx = i
            for j in range(i+1, len(coord_atan)):
                if coord_atan[min_idx] < coord_atan[j]:
                    min_idx = j
            coord_atan[i], coord_atan[min_idx] = coord_atan[min_idx], coord_atan[i]
            coords[i], coords[min_idx] = coords[min_idx], coords[i]

        individual[polygon_index][1:] = coords
    elif 0.4 < roulette < 0.6:
        # mutate points
...
def run(population=75, survival_rate=0.1, mutation_rate=0.01, generations=5000,
    ...
    path = os.path.join(os.getcwd(), "v2output")
