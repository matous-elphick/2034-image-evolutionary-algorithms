...
DESCRIPTION
    ...
    This is the solution representation variant algorithm, which is a variant
    for what shapes are used and how they are stored.

    Solution Representation:
        - Initially 10 rectangles in an individual
    Mutation:
...
def draw(individual, save=False, generation=-1, path=None):
    ...
    for polygon in individual:
        ...
        canvas.rectangle(polygon[1:], fill=polygon[0])
...
def run(population=75, survival_rate=0.1, mutation_rate=0.01, generations=10000,
    ...
    :param target_image: File path of target image
    ...
    path = os.path.join(os.getcwd(), "v1output")
